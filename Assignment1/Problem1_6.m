...
for i = 1:N+1
   ...
   Phi_d = 0;
   Theta_d = -1.5*sin(0.1*t)*deg2rad;
   Psi_d = 0.5*cos(0.05*t)*deg2rad;
   
   w_d = inv(Tzyx(phi_d,theta_d))*[Phi_d;Theta_d;Psi_d];
   ...
   tau = -[k_p*eye(3), k_d*eye(3)]*[q_prod(2:4) ;w-w_d];  % control law
...
xlabel('time [s]'); 
ylabel('Error [deg]');
saveas(gcf,'./figures/Error1_6.pdf')
...
xlabel('time [s]'); 
ylabel('angle [deg]');
saveas(gcf,'./figures/EulerAngles1_6.pdf')
...
saveas(gcf,'./figures/EulerVelocities1_6.pdf')
...
xlabel('time [s]'); 
ylabel('input [Nm]'); 
saveas(gcf,'./figures/ControlInput1_6.pdf')