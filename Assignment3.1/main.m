...
%
% Author:           Mathias Thoresen Paasche
% Study program:    MTTK
...
%psi_ref = ones(Ns+1,1);
%psi_ref(1:Ns/2) = psi_ref(1:Ns/2).*10*pi/180;
%psi_ref(Ns/2+1:end) = psi_ref(Ns/2+1:end).*25*pi/180;
...
u_ref = ones(Ns+1,1);
u_ref(1:Ns/2) = u_ref(1:Ns/2).*7;
u_ref(Ns/2+1:end) = u_ref(Ns/2+1:end).*14;
